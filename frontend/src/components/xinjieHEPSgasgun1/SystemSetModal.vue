      ...
      <div class="settings-scroll">
        <div class="settings-section">
          ...
          <div class="settings-grid-2col">
            <div v-for="field in yFields" :key="field.key" class="setting-group">
              <label>{{ field.label }}(Y):</label>
              <input v-model.number="config[field.key]" type="number" />
            </div>
          </div>
        </div>
        ...
        <div class="settings-section">
          ...
          <div class="settings-grid-2col">
            <div v-for="field in dFields" :key="field.key" class="setting-group">
              <label>{{ field.label }}(D):</label>
              <input v-model.number="config[field.key]" type="number" />
            </div>
          </div>
        </div>
        ...
        <div class="settings-section">
          <h4 class="settings-section-title">辅助点位配置</h4>
          <div class="settings-grid-2col">
            <div v-for="field in mFields" :key="field.key" class="setting-group">
              <label>{{ field.label }}(M):</label>
              <input v-model.number="config[field.key]" type="number" />
            </div>
          </div>
        </div>
...
const yFields = [
  { key: 'InletAddr', label: '进气阀' }, { key: 'OutletAddr', label: '出气阀' }, { key: 'FireAddr', label: '发射阀' },
