...
func (this *XinjieGasGun1) heartbeat() {
	...
	for {
		...
		vacuumFloat, err := this.plc.ReadRegister(XinJie.D(VacuumFloatAddr), XinJie.Float32, false)
		if err != nil {
		...
		pressureFloat, err := this.plc.ReadRegister(XinJie.D(PressureFloatAddr), XinJie.Float32, false)
		if err != nil {
			this.AddMessageToFrontend(err.Error())
			this.DisconnectDevice()
			return
		...
		this.heartbeat_info.OUTPUT["Fire"] = Y[FireAddr]
		this.heartbeat_info.OUTPUT["Inlet"] = Y[InletAddr]
		this.heartbeat_info.OUTPUT["Outlet"] = Y[OutletAddr]
		this.heartbeat_info.OUTPUT["VacuumRealse"] = Y[VacuumRealseAddr]
		// this.heartbeat_info.OUTPUT["PressureOpen"] = Y[PressureOpenAddr]
		// this.heartbeat_info.OUTPUT["PressureClose"] = Y[PressureCloseAddr]
		this.heartbeat_info.OUTPUT["TailVacuumPump"] = Y[TailVacuumPumpAddr]
		this.heartbeat_info.OUTPUT["TarVacuumPump"] = Y[TarVacuumPumpAddr]
		...
		if time.Since(lastDog) >= 2*time.Second {
			dogState = !dogState
			if err := this.plc.WriteCoil(XinJie.M(DogAddr), dogState); err == nil {
				lastDog = time.Now()
			}
		}
...
func (g *XinjieGasGun1) InletSwitch(open bool) error {
	return g.plc.WriteCoil(XinJie.Y(InletAddr), open)
}

// 排气开关
func (g *XinjieGasGun1) ExhaustSwitch(open bool) error {
	return g.plc.WriteCoil(XinJie.Y(OutletAddr), open)
}
...
func (g *XinjieGasGun1) FireSwitch(ms int) error {
	...
	err := g.plc.WriteCoil(XinJie.Y(FireAddr), true)
	if err != nil {
		return err
	}
	...
	go func(delayMs int) {
		...
		err := g.plc.WriteCoil(XinJie.Y(FireAddr), false)
		if err != nil {
...
func (g *XinjieGasGun1) VacuumSwitch(open bool) error {
	return g.plc.WriteCoil(XinJie.Y(TarVacuumPumpAddr), open)
}

// 抽尾部真空开关
func (g *XinjieGasGun1) TailVacuumSwitch(open bool) error {
	g.plc.WriteCoil(XinJie.Y(VacuumRealseAddr), open)
	time.Sleep(1000 * time.Millisecond)
	return g.plc.WriteCoil(XinJie.Y(TailVacuumPumpAddr), open)
}

// 增压泵开关
func (g *XinjieGasGun1) PressureSwitch(open bool) error {
	if open {
		err := g.plc.WriteCoil(XinJie.Y(PressureCloseAddr), true)
		if err != nil {
			return err
		}
		err = g.plc.WriteCoil(XinJie.Y(PressureOpenAddr), true)
		if err != nil {
			return err
		}

		time.Sleep(500 * time.Millisecond)
		err = g.plc.WriteCoil(XinJie.Y(PressureOpenAddr), false)
		if err != nil {
			return err
		}
		...
		err = g.plc.WriteCoil(XinJie.Y(PressureCloseAddr), false)
		if err != nil {
			return err
		}
